...
ns.setBufferTime(0);
  ns.play("music/1.flv"); //old way... lets see other options //for now
...
var ds_ = new DropShadowFilter();
		...
		//no json in as2
		var navplace = {};
		navplace.home = [310,120];
		navplace.music = [620,140];
		navplace.tours = [620,140];
		navplace.press = [35,140];
		navplace.about = [620,140];
		navplace.photos = [620,140];
		navplace.contact = [35,140];



		var headerplace = {}
		headerplace.home = [360,-40];
		headerplace.music = [675,-20];
		headerplace.tours = [675,-20];
		headerplace.press = [100,-20];
		headerplace.about = [675,-20];
		headerplace.photos = [675,-20];
...
if (page == "photos")
{
photos.loadMovie("viewer.swf");

			//photos.createEmptyMovieClip("grid", 999)
			
			/*photos.grid.beginFill(0xFF0000, 100);
			photos.grid.moveTo(0, 0);
			photos.grid.lineTo(nav_width, 0);
			photos.grid.lineTo(nav_width, nav_height);
			photos.grid.lineTo(0, nav_height);
			photos.grid.endFill();*/
		}
...
*/
 ...
 var abouts = [
 "        Cindy knows and loves the feel of walking on a stage.  The lights are low and the crowd cheers.  When she hears her name and the music starts to play, anything and everything was worth the moment.  It only takes one look at her fiery red hair, huge smile and the groove of the music to feel just like she does.  You are certain she is loving it...it shows in every note she sings and every move she makes.  She has a lifelong love of country music, never misses a chance to do a smooth love song and loves to rock the house down.  Whether it’s an acoustic set with a softer feel or the whole band is on the stage, you know a lot about her through her songs.  She has emerged as a songwriter in recent years and she writes of her personal life freely.  You feel the passion as she tells you about her \"sweet husband\".  She usually points him out in the crowd and he shyly smiles and looks proudly at her.  As they interact, the songs explain themselves.  She puts on a show that you are most likely never to forget. "
,
"Cindy has worked with incredible artists.  She will always tell you that the bigger the artist, the nicer they are…..and they are always nice to her.  Randy Travis, Merle Haggard, George Jones, Clay Walker, Neal McCoy, Luke Bryan and Steve Holy to name a few.  She also enjoys the festivals and the headliners that go along with them.  The crowds are big and the atmosphere is made for her.  At every show, she takes more time selling and signing CD’s than she spends performing on the stage.  Everyone feels they know her when they go home. " 
+ "\n\n" +
"        Her humanitarian spirit comes through at every performance as she tells about why she gives 20% of the profit from her CD “Same Red Hair” to the Fisher House. These homes provide everything a person, or family, needs while their loved one, coming back from the war is cared for in the nearby Veterans Hospital."
,
" She has taken her band to Hawaii twice to do benefit concerts at the Tripler Army Medical Center and has grown to appreciate all the Fisher House stands for. "
+ "\n\n" +
 "        As a young girl, she made the trip to Nashville.  She knocked on the right door at the right time, and found herself in the office of Shelby Singleton, owner of Sun Records.  The next morning she found herself performing on the “Ralph Emery Morning Show”.  And the rest is history..... " 
 ]
...
var go_about = function(){
    ...
    //purchase.loadMovie("purchase.png");
		...
		var about_fmt = new TextFormat("eaves", 26);
		...
		about_txt.text = abouts[about_index]
		//header_txt.autoSize = true;
		...
		about_next_txt.setTextFormat(about_fmt);
      ...
      about_prev_txt.setTextFormat(about_fmt);
...
var go_tours = function(){
    ...
    //tours_css.load("styles.css");
		...
		tours_txt.htmlText =  tourss[tours_index]
		//header_txt.autoSize = true;
...
"<a href = 'http://www.azstarnet.com/sn/metro/202677.php'>Country fans soak it in as festival ends long 'drought'" + "\n" +
"September 23, 2007,</a>" +"\n\n" + 

"<a href = 'http://www.randolphmichaud.com/charts.htm'>Cindy Standage #1 on Roots Airplay Top 100" + "\n" +
"April 05, 2008" + "\n\n" +
...
/*
   ...
   */
    ...
    press_txt.html = true; //comment this html try
		...
		press_next_txt.setTextFormat(press_fmt_gray); //change this if more
...
*/
	...
	var titles = ["Deeper Blue (Same Red Hair)",
...
var go_music = function()
{
    ...
    for (var i = 0; i < songs.length; i++)
    {
      ...
      music_lines[i].createTextField("music_txt",music_lines[i].getNextHighestDepth(),0,i * 35,280,100);
      
      
      var music_fmt = new TextFormat("eaves",28); //change to eaves
      music_fmt.color=0xffffff;
      music_fmt.align = "left";
      
      var music_fmt_over = new TextFormat("eaves", 28);
      ...
      music_lines[i].onPress = function()
      {
          ...
          demo_sound.setVolume(100);
                ...
                //demo_sound.loadSound("music/1.mp3",true);
      }
  }
}
...
*/
        ...
        if (page == "tours" && this.navtext.text.toLowerCase() != "tours")
          ...
          var myVolume:Volume = new Volume(videoSound);
            myVolume.animationStyle(3000,Quad.easeIn,"soundFadedIn");
            myVolume.run(100);
            ...
            */
            ...
            music.unloadMovie(music);
             ...
             demo_sound.stop();
				...
				this.navtext.text.setTextFormat(nav_fmt_over);
        ...
        if (page == "music")
        {
         ...
         var myVolume:Volume = new Volume(videoSound);
            myVolume.animationStyle(3000,Quad.easeIn,"what");
            myVolume.run(0);
         videoSound.setVolume(0)
...
*/
		var images = ["blogger.png","myspace.jpg","fisherhouse.png"]
